"""
Mirrorverse - Self-Hosted Markdown Knowledge Base
...
async def login_page(request: Request, error: str = None):
    ...
    content = content.replace('Mirrorverse', app_name)
...
async def create_share(request: Request, note_path: str, data: dict = None):
    ...
    try:
        ...
        # Get theme from request body (default to cyber)
        theme = "cyber"
...
async def view_shared_note(request: Request, token: str):
    ...
    try:
        ...
        theme = share_info.get('theme', 'cyber')
        ...
        if not theme_css:
            theme_css = get_theme_css(str(themes_dir), "cyber")
            theme = "cyber"
        
        # Strip data-theme selector
        theme_css = theme_css.replace(f':root[data-theme="{theme}"]', ':root')
        theme_css = theme_css.replace(':root[data-theme="cyber"]', ':root')
...
async def catch_all(full_path: str, request: Request):
    ...
    async with aiofiles.open(index_path, 'r', encoding='utf-8') as f:
        content = await f.read()
    app_name = config['app']['name']
    return content.replace('<title>Mirrorverse</title>', f'<title>{app_name}</title>')
